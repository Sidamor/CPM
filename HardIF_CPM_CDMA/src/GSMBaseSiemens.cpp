...
{   
	...
	*/
	...
	//查询SMS支持格式
	if (!(ret = DoCmd("AT\r", (char*)"OK", NULL, nLen, 0, 500)))
	{
		return ret;
	}
	//回显设置，需要回显
	...
	//检查R-UIM卡是否插好
	if (!(ret = DoCmd("AT+CPIN?\r", (char*)"OK", NULL, nLen, 0, 500)))
	{
		return ret;
	}
	
	//短消息发送格式，1-TEXT，0-PDU
	if (!(ret = DoCmd("AT+CMGF=1\r", (char*)"OK", NULL, nLen, 0, 500)))
	{
		return ret;
	}
	...
	*/
	...
	if (!(ret = DoCmd("AT+CNMI=1,1,0,2,0\r", (char*)"OK", NULL, nLen, 0, 500)))
	{
		return ret;
	}
...
//	DBG_GSM(("\n------------------------------GSM:发送短信--------------------------------\n"));
	...
	//短消息参数  1,英文; 6,UNICODE;
	if (!DoCmd("AT^HSMSSS=0,0,6,0\r", (char*)"OK", NULL, nLen, 0, 500))
	{
...
//	AddMsg((unsigned char*)content, strlen(content));
	...
	int contentLen = GB2UniCode(content, (unsigned char*)szATBuf);
	
	
//	DBG_GSM(("---szATBuf---\n"));
//	AddMsg((unsigned char*)szATBuf, contentLen);
	for(int i=0;i<contentLen;i++ ) 
	{
		strContent += (char)szATBuf[i];
	}
//	DBG_GSM(("---strContent---\n"));
//	AddMsg((unsigned char*)strContent.c_str(), contentLen);


	if( strContent.size()/2 <= 70)//非长短信，直接发送
	{		
...
//		sprintf(szLen, "%02x", telLen);
//		strTel = szLen;
//		strTel = "001100" + strTel;
//		strTel += "91";
//		strTel += szTel;
//		strTel += "000801";
...
//		sprintf(szATMsg, "AT^HCMGS=%d\r", nLen);//lhy 短信收发格式确认	
		
		char szATTel[64] = {0};
		memset(szATTel, 0, sizeof(szATTel));
		sprintf(szATTel, "AT^HCMGS=\"%s\"\r", tel);//lsd 目的手机号
		if (!DoCmd(szATTel, (char*)">", NULL, nLen, 0, 500))
		{
			return SYS_STATUS_FAILED;
		}
...
//		DBG_GSM(("---strContent---\n"));
//		AddMsg((unsigned char*)strContent.c_str(), contentLen+2);	
		
		//DoCmdEx因为最后为0x00 0x1a  函数中，发送时计算长度的时候需要+2
		if (!DoCmdEx(strContent.c_str(), contentLen+2, (char*)"OK", NULL, nLen, 0, 500))		
		{
...
{ 
	...
	char reqBuf[50] = {0};
	sprintf(reqBuf, "AT^HCMGL=%d\r", smsType);
...
int CGSMBaseSiemens::DoPhoneCall(char* Tel, char* soundPos)
{
	...
	//语音通道选择。默认通道0，选择通道1；
	if (!(ret = DoCmd("AT^SWSPATH=1\r", (char*)"OK", NULL, nLen, 0, 500)))
	{
		return ret;
	}
...
bool CGSMBaseSiemens::DoCmdEx(const char* ptrResq, int reqLen, char* checkValue, char* outBuf, int& outLen, int timeoutSec, int timeoutMill)
{
	bool ret = false;
	unsigned char buf[1400] = {0};
	unsigned char* ptrBuf = (unsigned char*)outBuf;
	...
	DBG_GSM(("[%s][%d]\n", ptrResq, reqLen));
	if (ComSend((unsigned char*)ptrResq, reqLen))
	{
    DBG_GSM(("Sms comm recv---%d-->\n", reqLen));
		if (0 < (outLen = ComRecv(ptrBuf,needLen, timeoutSec, timeoutMill, false)))
		{
			if (NULL == checkValue || NULL != strstr((char*)ptrBuf, checkValue))
			{
...
//	DBG_GSM(("Recv Comm handle[%d]\n", m_hFd));
	...
	else if (NULL != strstr((char*)oubBuf, "^CEND") || NULL != strstr((char*)oubBuf, "NO ANSWER"))//
	{
		//关闭放音
